...
## TOML の canonical 形式（key/table の一貫化）

TOML は YAML ほど寛容ではありませんが、学習データ側に **複数の出力スタイル**（キー順序や `[table]` / `[[table]]` の並べ方など）が混在すると、モデルが「ごちゃまぜ」の書き方を学習してしまうことがあります。

本プロジェクトでは以下を **TOML の canonical 形式**として扱い、学習データと GRPO reward の両方で揃える。
...
- 変換/正規化: `src/data/toml_canonical.py`
  - `canonicalize_toml_text()` / `dumps_toml_canonical()`
- 判定: `src/data/validators.py::toml_is_canonical()`
- GRPO 報酬: `src/rl/rewards.py`
  - `toml_canonical` コンポーネント
  - `w_toml_canonical`, `p_toml_canonical_fail`
