...
`scripts/run_sft_hf.sh` / `scripts/run_grpo_hf.sh` は、HF 変換後に
`python -m src.data.hf_select_subset` を呼び出して **サブセット選択**を行う。
（HF データは改変せず、**選択のみ**）

設定は `configs/sft_hf.yaml` / `configs/grpo_hf.yaml` の `data.sampling` に集約する。

- `data.sampling.mode`
  - `balance_by_task`: `task_key` ごとに均一サンプリング（従来デフォルト）
  - `per_output_type`: 形式（JSON/YAML/TOML/XML/CSV）ごとに件数指定
  - `none`: 何もせず、そのまま使用

#### mode=balance_by_task

`data.sampling.balance_by_task` を参照し、内部で `src.data.balance_by_task` と同等の処理を行う。

- `enabled`: true/false
- `strategy`: `min|max|fixed`
- `fixed_n`: strategy=fixed の場合の件数
- `seed`: シャッフル seed
- `min_count`: 極端に少ない task_key グループを除外してからバランスする

> 互換性のため、`HF_BALANCE_*` 環境変数が設定されている場合は、
> `balance_by_task.*` を **環境変数が上書き**する。

#### mode=per_output_type

`data.sampling.per_output_type.targets` で形式ごとに件数を指定する。

- 値は `all` または整数
- `default_target: null` の場合、targets に無い形式は **全件**
- `oversample_with_replacement: true` にすると、指定数が母数を超えたときに重複ありで埋める

例（TOML を全件・他形式は上限を設ける）:
